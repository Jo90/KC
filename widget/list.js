...
YUI().add('j-widget-list',function(Y){
    ...
    List.ATTRS={
        ...
        title:{value:'Select tags from the dropdown list'}
    };
    ...
    Y.extend(List,Y.Widget,{
        ...
        renderUI:function(){
            ...
            Y.each(this.get('elements'),function(el){
                elementsArr.push(
                    ...
                    +' value="'+el.id+'" title="click to remove">'
                    +  el.name
                    +'</button>'
                );
                selectArr.push(
            ...
            nodeSelect=Y.Node.create(
                '<select title="'+title+'">'
               +  '<option><strong>'+this.get('selectorPrompt')+'</strong></option>'
               +  selectArr.join('')
               +'</select>'
            );
            if(this.get('nodeSelectBeforeElements')){
        ...
        _el:{
            add:function(){
                ...
                selected.push(nodeSelect.get('value'));
            ...
            selectedHasChanged:function(e){
                ...
                nodeSelect.all('option').each(function(option,idx){
                    ...
                    if(Y.Array.indexOf(selected,option.get('value'))!==-1){
    ...
    Y.namespace('J.widget').List=List;
